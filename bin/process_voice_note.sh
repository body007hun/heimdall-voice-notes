...
on_error() {
  local exit_code="$?"
  log "ERROR" "Processing failed: note_id=${note_id:-unknown}, exit_code=$exit_code"

  if [[ -n "${work_file:-}" && -f "$work_file" ]]; then
    mkdir -p "$FAILED_DIR"
    mv -f "$work_file" "$FAILED_DIR/$(basename "$work_file").failed"
    log "ERROR" "Moved work file to failed/: $(basename "$work_file").failed"
  fi
...
log "INFO" "Start processing: input=$input_base note_id=$note_id model=$MODEL_NAME language=$LANGUAGE source=$SOURCE_NAME"

if [[ "$input_abs" == "$INBOX_DIR/"* ]]; then
  mv -f "$input_abs" "$work_file"
  log "INFO" "Moved inbox file to processing: $(basename "$work_file")"
else
  cp -f "$input_abs" "$work_file"
  log "INFO" "Copied external file to processing: $(basename "$work_file")"
fi
...
[[ -f "$raw_txt_tmp" ]] || die "Whisper txt output missing: $raw_txt_tmp"
...
if command -v ffprobe >/dev/null 2>&1; then
  duration="$(ffprobe -i "$work_file" -show_entries format=duration -v quiet -of csv='p=0' 2>/dev/null || true)"
fi
...
log "INFO" "Done: note=${note_final#$BASE/} audio=${audio_final#$BASE/} raw=${raw_txt_final#$BASE/} elapsed=${elapsed}s"
